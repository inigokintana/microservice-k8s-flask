...
@swag_from('./api/v1/task-id-put.yaml')
def update_task(id):
    # Extract the API key from the header in the HTTP request
    header_api_key = request.headers.get('x-access-token')
...
@swag_from('./api/v1/task-id-del.yaml')
def delete_task(id):
    # Extract the API key from the header in the HTTP request
    header_api_key = request.headers.get('x-access-token')
...
@swag_from('./api/v1/task-del-all.yaml')
